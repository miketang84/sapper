...
use http::StatusCode;
use may_http::server::{HttpServer, HttpService, Request, Response};
        ...
        match response_w {
                ...
                if self.static_service {
                    match simple_file_get(&path) {
                        Ok((file_u8vec, file_mime)) => {
                            res.headers_mut()
                                .insert("Content-Type", file_mime.parse().unwrap());
            ...
            Err(Error::TemporaryRedirect(new_uri)) => {
                *res.status_mut() = StatusCode::TEMPORARY_REDIRECT;
                res.headers_mut()
                    .insert("Location", new_uri.parse().unwrap());
            ...
            Err(Error::CustomJson(json_str)) => {
                *res.status_mut() = StatusCode::OK;
                res.headers_mut()
                    .insert("Content-Type", "application/x-javascript".parse().unwrap());
