...
pub use sapper_session::{set_cookie, SessionVal};
pub use sapper_tmpl::{render, to_value, Context as WebContext, TeraResult, TeraValue, TERA};
...
macro_rules! res_redirect {
    ($redirect_uri:expr) => {{
        ...
        response
            .headers_mut()
            .set(Location($redirect_uri.to_owned()));
...
macro_rules! res_xml_string {
    ($context:expr) => {{
        ...
        res.headers_mut()
            .set_raw("Content-Type", vec!["text/xml".as_bytes().to_vec()]);
...
macro_rules! _using_default {
    ($field:expr, $default:expr) => {{
        info!(
            "missing or unrecognized parameter {}, using default {}.",
            $field, $default
        );
        // return default
        $default
    ...
    ($params:expr, $field:expr, $tykey:ty, $default:expr) => {{
        match $params.get($field) {
            Some(ref astr) => {
                ...
                match _t.parse::<$tykey>() {
                    Ok(output) => output,
                    Err(_) => _using_default!($field, $default),
                }
            }
            None => _using_default!($field, $default),
        }
