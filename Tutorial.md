...
Sapper 最大的特点是把 web 业务分为了两个层次（全局、模块）去处理，每个模块都可以有自己的路由和中间件，全局可以定义全局共享的中间件和全局变量。把颗粒度放到最细就意味着每一个请求都可以有自己的中间件。
...
#### 代码解说
现在，demo 项目可以通过 `cargo run` 命令启动，监听在 `127.0.0.1:8080`。可以通过 `crul` 或者 Python 库 httpie 的命令 `http` 进行测试。 
